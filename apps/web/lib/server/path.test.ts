...
describe('levelStates', () => {
  ...
  it('keeps completed levels after the current one completed, and shows legendary', () => {
    const s = levelStates(
      [lvl('a'), lvl('b'), lvl('c')],
      new Map([done('a', { legendary: true }), done('c')]),
    )
  ...
  it('never blocks the path on levels without an MVP session (chest, story)', () => {
    const s = levelStates(
      [lvl('a'), lvl('chest', 'chest'), lvl('b'), lvl('story', 'story')],
      new Map([done('a')]),
    )
    expect([...s.values()].map((x) => x.state)).toEqual([
      'completed',
      'available',
      'current',
      'locked',
  ...
  it('reports partial progress and caps lessonsDone at the total', () => {
    const partial = ['a', { ...done('a')[1], completedAt: null, lessonsDone: 2 }] as const
    const s = levelStates([lvl('a', 'lesson', 4)], new Map([partial]))
    expect(s.get('a')).toEqual({ state: 'current', lessonsDone: 2 })
    expect(
      levelStates([lvl('x', 'lesson', 1)], new Map([done('x', { lessonsDone: 9 })])).get('x'),
    ).toEqual({
      state: 'completed',
      lessonsDone: 1,
    })
...
describe('path migrations', () => {
  ...
  it('chains the steps between two versions, skipping versions without a migration', () => {
    expect(migrationSteps(manifest, 1, 5).map((m) => `${m.from}->${m.to}`)).toEqual([
      '1->2',
      '3->5',
    ])
