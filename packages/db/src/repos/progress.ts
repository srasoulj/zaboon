 ...
 */
export async function appendXp(
  tx: Tx,
  userId: string,
  entry: NewXpEntry,
...
export async function getDailyActivity(
  ...
  const [row] = await tx
    ...
    .where(
      and(eq(schema.dailyActivity.userId, userId), eq(schema.dailyActivity.localDate, localDate)),
    )
...
export async function markFreezeUsed(
  tx: Tx,
  userId: string,
