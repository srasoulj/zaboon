...
describe('profiles repository', () => {
  ...
  it('ensureProfile is idempotent', async () => {
    ...
    const rows =
      await ctx.admin`SELECT count(*)::int AS n FROM public.profiles WHERE user_id = ${alice}`
  ...
  it('updates profile fields', async () => {
    ...
    const p = await withUserLock(ctx.h.db, alice, (tx) =>
      profiles.updateProfile(tx, alice, {
  ...
  it('merges settings patches over defaults and validates them', async () => {
    const s1 = await withUser(ctx.h.db, alice, (tx) =>
      profiles.updateSettings(tx, alice, { sound: false, dailyGoalXp: 30 }),
    )
    expect(s1).toEqual({ ...DEFAULT_SETTINGS, sound: false, dailyGoalXp: 30 })
    const s2 = await withUser(ctx.h.db, alice, (tx) =>
      profiles.updateSettings(tx, alice, { motion: 'reduced' }),
    )
    ...
    await expect(
      withUser(ctx.h.db, alice, (tx) =>
        profiles.updateSettings(tx, alice, { motion: 'wild' as never }),
      ),
    ).rejects.toThrow()
  })
  ...
  it('rejects a taken username with ConflictError and keeps the transaction usable', async () => {
    await withUser(ctx.h.db, alice, (tx) =>
      profiles.updatePublicProfile(tx, alice, { username: 'alice_1', displayName: 'Alice' }),
    )
    await expect(
      withUser(ctx.h.db, bob, (tx) =>
        profiles.updatePublicProfile(tx, bob, { username: 'alice_1' }),
      ),
    ).rejects.toBeInstanceOf(ConflictError)
    // Setting your own current username again is fine.
    const same = await withUser(ctx.h.db, alice, (tx) =>
      profiles.updatePublicProfile(tx, alice, { username: 'alice_1' }),
    )
  ...
  it('lists public profiles of other users (readable by design) and syncs stats', async () => {
    await withUser(ctx.h.db, alice, (tx) =>
      profiles.syncPublicStats(tx, alice, { xpTotal: 42, streakCurrent: 3 }),
    )
    const list = await withUser(ctx.h.db, bob, (tx) =>
      profiles.listPublicProfiles(tx, [alice, bob]),
    )
    expect(list.find((p) => p.userId === alice)).toMatchObject({
      username: 'alice_1',
      xpTotal: 42,
      streakCurrent: 3,
    })
  ...
  describe('cross-user access (IDOR)', () => {
    it("a user-scoped transaction cannot read or change another user's private profile", async () => {
      ...
      expect(
        await withUser(ctx.h.db, alice, (tx) =>
          profiles.updateProfile(tx, bob, { onboarded: true }),
        ),
      ).toBeNull()
      expect(
        await withUser(ctx.h.db, alice, (tx) => profiles.updateSettings(tx, bob, { sound: false })),
      ).toBeNull()
      expect(
        await withUser(ctx.h.db, alice, (tx) =>
          profiles.updatePublicProfile(tx, bob, { displayName: 'x' }),
        ),
      ).toBeNull()
      const bobNow = await withSystem(ctx.h.db, (tx) => profiles.getProfile(tx, bob))
      expect(bobNow?.onboarded).toBe(false)
      expect(await withSystem(ctx.h.db, (tx) => profiles.getPublicProfile(tx, bob))).toMatchObject({
        displayName: null,
      })
    })
    ...
    it("cannot set another user's consent", async () => {
      await expect(
        withUser(ctx.h.db, alice, (tx) => profiles.setConsent(tx, bob, 'analytics', true)),
      ).rejects.toThrow()
    })
  })
})
