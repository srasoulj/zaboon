...
describe('account export', () => {
  it("contains every user-owned table and only the caller's rows", async () => {
    ...
    for (const t of ['profiles', 'public_profiles', 'sessions', 'session_events', 'session_answers', 'xp_ledger',
      'daily_activity', 'streaks', 'lives', 'enrollments', 'level_progress', 'lexeme_memory', 'letter_memory',
      'mistakes', 'reports', 'user_items', 'consents']) {
  ...
  it("cannot export another user's rows", async () => {
    await expect(withUser(ctx.h.db, alice, (tx) => account.exportAccount(tx, bob))).rejects.toBeInstanceOf(ScopeError)
  })
})
...
describe('account delete', () => {
  it("cannot delete another user's account", async () => {
    ...
    const rows = await ctx.admin`SELECT count(*)::int AS n FROM public.profiles WHERE user_id = ${bob}`
  ...
  it('deletes the profile and every row that belongs to the user', async () => {
    ...
    for (const table of await userTables()) {
      const rows = await ctx.admin.unsafe(`SELECT count(*)::int AS n FROM public."${table}" WHERE user_id = $1`, [alice])
    ...
    expect(await withUserLock(ctx.h.db, alice, (tx) => account.deleteAccount(tx, alice))).toBe(false)
  })
})
