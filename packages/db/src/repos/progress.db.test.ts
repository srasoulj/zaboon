...
describe('xp_ledger', () => {
  ...
  it('is idempotent on (session_id, reason)', async () => {
    const first = await withUserLock(ctx.h.db, alice, (tx) => progress.appendXp(tx, alice, entry(aliceSession, 'session', 10)))
    const replay = await withUserLock(ctx.h.db, alice, (tx) => progress.appendXp(tx, alice, entry(aliceSession, 'session', 10)))
    const bonus = await withUserLock(ctx.h.db, alice, (tx) => progress.appendXp(tx, alice, entry(aliceSession, 'perfect', 5)))
    ...
    const list = await withUser(ctx.h.db, alice, (tx) => progress.listXpLedger(tx, alice, { sessionId: aliceSession }))
  ...
  it('allows several entries without a session', async () => {
    ...
    expect(await withUser(ctx.h.db, u, (tx) => progress.getXpSince(tx, u, '2026-09-25T10:00:00Z'))).toBe(6)
    expect(await withUser(ctx.h.db, u, (tx) => progress.getXpSince(tx, u, '2026-09-25T11:00:00Z'))).toBe(0)
  })

  it('is append-only for app_server, even in system scope', async () => {
    await expect(withSystem(ctx.h.db, (tx) => tx.execute(sql`UPDATE public.xp_ledger SET amount = 1000`))).rejects.toMatchObject({
      cause: { code: '42501' },
    })
    await expect(withSystem(ctx.h.db, (tx) => tx.execute(sql`DELETE FROM public.xp_ledger`))).rejects.toMatchObject({
      cause: { code: '42501' },
    })
  })
  ...
  it("cannot credit XP against another user's session", async () => {
    await expect(
      withUserLock(ctx.h.db, alice, (tx) => progress.appendXp(tx, alice, entry(bobSession, 'session', 10))),
    ).rejects.toBeInstanceOf(NotFoundError)
    await expect(
      withUserLock(ctx.h.db, alice, (tx) => progress.appendXp(tx, bob, entry(bobSession, 'session', 10))),
    ).rejects.toThrow()
...
describe('daily_activity', () => {
  it('accumulates XP and sessions per local day; goal_met is sticky', async () => {
    const u = await ctx.newUser()
    await withUserLock(ctx.h.db, u, (tx) => progress.addDailyActivity(tx, u, { localDate: '2026-09-24', xp: 10, goalMet: true }))
    const d = await withUserLock(ctx.h.db, u, (tx) => progress.addDailyActivity(tx, u, { localDate: '2026-09-24', xp: 15 }))
    expect(d).toEqual({ localDate: '2026-09-24', xp: 25, sessions: 2, goalMet: true, freezeUsed: false })
    await withUserLock(ctx.h.db, u, (tx) => progress.markFreezeUsed(tx, u, ['2026-09-22', '2026-09-23']))
    ...
    const ranged = await withUser(ctx.h.db, u, (tx) => progress.listDailyActivity(tx, u, { from: '2026-09-23', to: '2026-09-23' }))
    expect(ranged.map((x) => x.localDate)).toEqual(['2026-09-23'])
    expect(await withUser(ctx.h.db, u, (tx) => progress.getDailyActivity(tx, u, '2026-09-24'))).toMatchObject({ xp: 25 })
  ...
  it("is invisible to and unwritable by other users", async () => {
    ...
    await expect(
      withUserLock(ctx.h.db, alice, (tx) => progress.addDailyActivity(tx, bob, { localDate: '2026-09-25', xp: 500 })),
    ).rejects.toThrow()
    expect(await withUser(ctx.h.db, bob, (tx) => progress.getDailyActivity(tx, bob, '2026-09-25'))).toMatchObject({ xp: 5 })
  })
})
...
describe('streaks, lives and items', () => {
  it('saves and reads the streak', async () => {
    ...
    expect(await withUser(ctx.h.db, alice, (tx) => state.getStreak(tx, alice))).toEqual({ ...s, current: 4 })
  })
  ...
  it('adds and spends items without going negative', async () => {
    const u = await ctx.newUser()
    expect(await withUserLock(ctx.h.db, u, (tx) => state.addItem(tx, u, 'streak_freeze', 1))).toBe(1)
    expect(await withUserLock(ctx.h.db, u, (tx) => state.addItem(tx, u, 'streak_freeze', 1))).toBe(2)
    expect(await withUserLock(ctx.h.db, u, (tx) => state.addItem(tx, u, 'streak_freeze', -2))).toBe(0)
    expect(await withUserLock(ctx.h.db, u, (tx) => state.addItem(tx, u, 'streak_freeze', -1))).toBeNull()
    expect(await withUserLock(ctx.h.db, u, (tx) => state.addItem(tx, u, 'never_had', -1))).toBeNull()
  ...
  it("cannot read or write another user's state", async () => {
    await withUserLock(ctx.h.db, bob, (tx) => state.saveStreak(tx, bob, { current: 9, longest: 9, lastActiveDate: null, freezes: 0 }))
    ...
    await expect(
      withUserLock(ctx.h.db, alice, (tx) => state.saveStreak(tx, bob, { current: 0, longest: 0, lastActiveDate: null, freezes: 0 })),
    ).rejects.toThrow()
    await expect(withUserLock(ctx.h.db, alice, (tx) => state.addItem(tx, bob, 'streak_freeze', 5))).rejects.toThrow()
    expect(await withUserLock(ctx.h.db, alice, (tx) => state.addItem(tx, bob, 'streak_freeze', -1))).toBeNull()
    expect(await withUser(ctx.h.db, bob, (tx) => state.getStreak(tx, bob))).toMatchObject({ current: 9 })
  })
})
