...
describe('lexeme and letter memory', () => {
  it('round-trips FSRS cards exactly', async () => {
    ...
    await withUserLock(ctx.h.db, alice, (tx) => memory.upsertLexemeCards(tx, alice, [{ id: 'lx_salam', card: c, exposures: 1 }]))
    const [got] = await withUser(ctx.h.db, alice, (tx) => memory.getLexemeCards(tx, alice, ['lx_salam']))
  ...
  it('updates cards; exposures are kept when omitted and replaced when given', async () => {
    await withUserLock(ctx.h.db, alice, (tx) =>
      memory.upsertLexemeCards(tx, alice, [{ id: 'lx_salam', card: card({ reps: 2, stability: 4 }) }]),
    )
    let [got] = await withUser(ctx.h.db, alice, (tx) => memory.getLexemeCards(tx, alice, ['lx_salam']))
    expect(got).toMatchObject({ exposures: 1, card: { reps: 2, stability: 4 } })
    await withUserLock(ctx.h.db, alice, (tx) =>
      memory.upsertLexemeCards(tx, alice, [{ id: 'lx_salam', card: card({ reps: 3 }), exposures: 4 }]),
    )
  ...
  it('lists due cards, most overdue first', async () => {
    ...
    const due = await withUser(ctx.h.db, alice, (tx) => memory.listDueLexemes(tx, alice, '2026-09-25T00:00:00.000Z'))
  ...
  it('stores letter cards separately', async () => {
    ...
    const due = await withUser(ctx.h.db, alice, (tx) => memory.listDueLetters(tx, alice, '2026-09-25T00:00:00.000Z'))
  ...
  it("never exposes or accepts another user's cards", async () => {
    ...
    await expect(
      withUserLock(ctx.h.db, bob, (tx) => memory.upsertLexemeCards(tx, alice, [{ id: 'lx_salam', card: card({ reps: 99 }) }])),
    ).rejects.toThrow()
    const [mine] = await withUser(ctx.h.db, alice, (tx) => memory.getLexemeCards(tx, alice, ['lx_salam']))
...
describe('mistakes', () => {
  it('counts, resolves and re-opens mistakes', async () => {
    const u = await ctx.newUser()
    await withUserLock(ctx.h.db, u, (tx) => learning.recordMistakes(tx, u, ['lexeme:lx_a', 'lexeme:lx_a', 'sentence:s_1'], '2026-09-25T10:00:00.000Z'))
    await withUserLock(ctx.h.db, u, (tx) => learning.recordMistakes(tx, u, ['lexeme:lx_a'], '2026-09-25T11:00:00.000Z'))
    ...
    expect(await withUserLock(ctx.h.db, u, (tx) => learning.resolveMistakes(tx, u, ['lexeme:lx_a'], '2026-09-25T12:00:00Z'))).toBe(1)
    expect(await withUserLock(ctx.h.db, u, (tx) => learning.resolveMistakes(tx, u, ['lexeme:lx_a'], '2026-09-25T12:00:00Z'))).toBe(0)
    open = await withUser(ctx.h.db, u, (tx) => learning.listOpenMistakes(tx, u))
    expect(open.map((m) => m.itemRef)).toEqual(['sentence:s_1'])
    await withUserLock(ctx.h.db, u, (tx) => learning.recordMistakes(tx, u, ['lexeme:lx_a'], '2026-09-25T13:00:00.000Z'))
    open = await withUser(ctx.h.db, u, (tx) => learning.listOpenMistakes(tx, u))
    expect(open[0]).toMatchObject({ itemRef: 'lexeme:lx_a', timesWrong: 3, resolvedAt: null })
  })

  it("are private to their owner", async () => {
    await withUserLock(ctx.h.db, bob, (tx) => learning.recordMistakes(tx, bob, ['lexeme:lx_b'], '2026-09-25T10:00:00Z'))
    expect(await withUser(ctx.h.db, alice, (tx) => learning.listOpenMistakes(tx, bob))).toEqual([])
    expect(await withUserLock(ctx.h.db, alice, (tx) => learning.resolveMistakes(tx, bob, ['lexeme:lx_b'], '2026-09-25T11:00:00Z'))).toBe(0)
    await expect(
      withUserLock(ctx.h.db, alice, (tx) => learning.recordMistakes(tx, bob, ['lexeme:lx_c'], '2026-09-25T11:00:00Z')),
    ).rejects.toThrow()
  })
})
...
describe('level_progress', () => {
  it('counts lessons up to the level total and stamps completion once', async () => {
    const u = await ctx.newUser()
    const rec = (at: string) =>
      withUserLock(ctx.h.db, u, (tx) => learning.recordLessonDone(tx, u, { courseId: 'fixture', levelId: 'u01-l1', lessonsTotal: 2, at }))
    expect(await rec('2026-09-25T10:00:00.000Z')).toMatchObject({ lessonsDone: 1, completedAt: null })
    expect(await rec('2026-09-25T11:00:00.000Z')).toMatchObject({ lessonsDone: 2, completedAt: '2026-09-25T11:00:00.000Z' })
    expect(await rec('2026-09-25T12:00:00.000Z')).toMatchObject({ lessonsDone: 2, completedAt: '2026-09-25T11:00:00.000Z' })

    const single = await withUserLock(ctx.h.db, u, (tx) =>
      learning.recordLessonDone(tx, u, { courseId: 'fixture', levelId: 'u01-l2', lessonsTotal: 1, at: '2026-09-25T10:00:00.000Z' }),
    )
    expect(single).toMatchObject({ lessonsDone: 1, completedAt: '2026-09-25T10:00:00.000Z' })

    const leg = await withUserLock(ctx.h.db, u, (tx) =>
      learning.setLegendary(tx, u, { courseId: 'fixture', levelId: 'u01-l1', at: '2026-09-26T10:00:00.000Z' }),
    )
    expect(leg).toMatchObject({ legendary: true, lessonsDone: 2 })

    const jumped = await withUserLock(ctx.h.db, u, (tx) =>
      learning.setLevelProgress(tx, u, { courseId: 'fixture', levelId: 'u02-l1', lessonsDone: 4, completed: true, at: '2026-09-26T10:00:00.000Z' }),
    )
    ...
    expect(await withUser(ctx.h.db, u, (tx) => learning.getLevelProgress(tx, u, 'fixture', 'u01-l2'))).toMatchObject({ lessonsDone: 1 })
  })

  it("is private to its owner", async () => {
    await withUserLock(ctx.h.db, bob, (tx) =>
      learning.recordLessonDone(tx, bob, { courseId: 'fixture', levelId: 'u01-l1', lessonsTotal: 3, at: '2026-09-25T10:00:00Z' }),
    )
    expect(await withUser(ctx.h.db, alice, (tx) => learning.getLevelProgress(tx, bob, 'fixture', 'u01-l1'))).toBeNull()
    await expect(
      withUserLock(ctx.h.db, alice, (tx) =>
        learning.recordLessonDone(tx, bob, { courseId: 'fixture', levelId: 'u01-l1', lessonsTotal: 3, at: '2026-09-25T10:00:00Z' }),
      ),
    ).rejects.toThrow()
    expect(await withUser(ctx.h.db, bob, (tx) => learning.getLevelProgress(tx, bob, 'fixture', 'u01-l1'))).toMatchObject({ lessonsDone: 1 })
  })
})
