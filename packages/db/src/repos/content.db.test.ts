...
describe('content_versions', () => {
  it('publishes versions and switches the current one (one per course)', async () => {
    await withSystem(ctx.h.db, async (tx) => {
      await content.publishContentVersion(tx, {
        courseId: 'fixture',
        version: 1,
        bundlePath: '/v1/fixture',
      })
      await content.publishContentVersion(tx, {
        courseId: 'fixture',
        version: 2,
        bundlePath: '/v2/fixture',
        minAppVersion: '0.2.0',
      })
      await content.publishContentVersion(tx, {
        courseId: 'fixture',
    ...
    expect(
      await withSystem(ctx.h.db, (tx) => content.setCurrentContentVersion(tx, 'fixture', 1)),
    ).toBe(true)
    expect(
      await withSystem(ctx.h.db, (tx) => content.setCurrentContentVersion(tx, 'fa-en', 1)),
    ).toBe(true)
    expect(
      await withSystem(ctx.h.db, (tx) => content.setCurrentContentVersion(tx, 'fixture', 2)),
    ).toBe(true)
    expect(
      await withSystem(ctx.h.db, (tx) => content.setCurrentContentVersion(tx, 'fixture', 9)),
    ).toBe(false)

    const cur = await withUser(ctx.h.db, alice, (tx) =>
      content.getCurrentContentVersion(tx, 'fixture'),
    )
    ...
    expect(
      (await content.listContentVersions(ctx.h.db, 'fixture')).map((v) => [v.version, v.isCurrent]),
    ).toEqual([
      [2, true],
      [1, false],
    ])
    expect(await content.getContentVersion(ctx.h.db, 'fixture', 1)).toMatchObject({
      bundlePath: '/v1/fixture',
    })
  })
  ...
  it('writes require system scope (repository check and RLS)', async () => {
    await expect(
      withUser(ctx.h.db, alice, (tx) =>
        content.publishContentVersion(tx, { courseId: 'x', version: 1, bundlePath: '/x' }),
      ),
    ).rejects.toBeInstanceOf(ScopeError)
    await expect(
      withUser(ctx.h.db, alice, (tx) =>
        tx.execute(
          sql`INSERT INTO public.content_versions (course_id, version, bundle_path) VALUES ('x', 1, '/x')`,
        ),
      ),
    ).rejects.toMatchObject({ cause: { code: '42501' } })
    await expect(
      withUser(ctx.h.db, alice, (tx) => content.setAppConfig(tx, 'hearts', {})),
    ).rejects.toBeInstanceOf(ScopeError)
  })
})
