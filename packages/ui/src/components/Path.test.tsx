...
import { asideIndex, PathLayout, PathNode, pathOffset, UnitBanner, unitColor, UNIT_COLORS, type PathUnit } from './Path'

describe('path geometry', () => {
  it('uses the token offsets and repeats them', () => {
    const offsets = tokens.path.offsets
    for (let i = 0; i < offsets.length * 2; i++) expect(pathOffset(i)).toBe(offsets[i % offsets.length])
...
describe('PathNode', () => {
  it('current: progress ring, START bubble, aria-current and progress in the name', () => {
    const { container } = render(<PathNode type="lesson" state="current" label="Lesson 2" progress={0.4} />)
  ...
  it.each(['locked', 'completed', 'legendary'] as const)('%s: no ring or bubble, state in the name', (state) => {
    const { container } = render(<PathNode type="chest" state={state} label="Reward" />)
    expect(screen.getByRole('button', { name: `Reward, ${state}` })).not.toHaveAttribute('aria-current')
  ...
  it('shows the type icon (lock for a locked lesson)', () => {
    const { container, rerender } = render(<PathNode type="review" state="completed" label="Unit review" />)
...
describe('UnitBanner', () => {
  it('renders section, unit, title heading and the guidebook button', async () => {
    ...
    render(<UnitBanner section={1} unit={2} title="Order at a café" color="lajvard" onGuidebook={onGuidebook} />)
...
describe('PathLayout', () => {
  const units: PathUnit[] = [
    ...
    { id: 'u2', section: 1, unit: 2, title: 'Food', nodes: [{ id: 'm0', type: 'lesson', state: 'locked', label: 'Level 1' }] },
  ]
  ...
  it('renders one labelled section per unit with offset rows from tokens', () => {
    ...
    expect(within(rows[2]!).getByText('hodhod').closest('.zb-path__aside')).toHaveAttribute('data-side', 'right')
  ...
  it('cycles unit colors and reports node clicks and guidebook taps', async () => {
    ...
    const onGuidebook = vi.fn()
    const { container } = render(<PathLayout units={units} onNodeClick={onNodeClick} onGuidebook={onGuidebook} />)
