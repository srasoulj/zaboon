...
export type ColorRef =
  { semantic: SemanticName } | { brand: BrandName; shade: '100' | '500' | '600' | 'label' }
...
const NEUTRAL_PAIRS: TextPair[] = [
  ...
  {
    use: 'selected choice card / tile text and hint',
    fg: s('ink'),
    bg: s('selected-bg'),
    large: false,
  },
  ...
  {
    use: 'ink text inside the correct feedback bar (solution)',
    fg: s('ink'),
    bg: s('correct-bg'),
    large: false,
  },
  {
    use: 'ink text inside the wrong feedback bar (solution)',
    fg: s('ink'),
    bg: s('wrong-bg'),
    large: false,
  },
  {
    use: 'locked / disabled Button3D label (19px/800)',
    fg: s('stone'),
    bg: s('line'),
    large: true,
  },
  ...
  {
    use: 'START bubble on the path (19px/800)',
    fg: { brand: 'firouzeh', shade: '500' },
    bg: s('bg'),
    large: true,
  },
]
...
export const FOCUS_RING = {
  band: { semantic: 'ink' } as ColorRef,
  halo: { semantic: 'bg' } as ColorRef,
}
...
export const FOCUS_RING_SURFACES: readonly FocusSurface[] = [
  ...
  ...BRAND_NAMES.map((brand) => ({
    use: `${brand}-100 tint`,
    surface: { brand, shade: '100' } as ColorRef,
  })),
]
...
export function focusRingContrast(
...
): { between: number; vsSurface: number } {
  ...
  return {
    between: contrastRatio(band, halo),
    vsSurface: Math.max(contrastRatio(band, bg), contrastRatio(halo, bg)),
  }
}
