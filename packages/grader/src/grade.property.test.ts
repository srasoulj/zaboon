...
describe('grader properties', () => {
  it('grade(compile([p]), canonical) is always correct (Persian)', () => {
    fc.assert(
      fc.property(pattern(FA_WORDS), fc.boolean(), (p, drop) => {
        ...
        expect(grade(g, c.split(' ').filter(Boolean), { lang: 'fa', mode: 'bank' }).verdict).toBe(
          'correct',
        )
      }),
      { numRuns: 400 },
  ...
  it('every enumerated answer is accepted, and the closest solution of a correct answer is accepted', () => {
    fc.assert(
      fc.property(
        pattern(FA_WORDS),
        fc.constantFrom(undefined, 'من آب می‌خواهم', 'ما چای می‌خوریم'),
        (p, formal) => {
          const g = compile([p], {
            lang: 'fa',
            ...(formal ? { formal } : {}),
  ...
  it('verdicts are never better than correct for noise, and the diff never drops solution words', () => {
    fc.assert(
      fc.property(
        pattern(EN_WORDS),
        fc.array(fc.constantFrom(...EN_WORDS, 'xyz', 'watr'), { maxLength: 8 }),
        (p, words) => {
          const g = compile([p], { lang: 'en' })
...
describe('grader performance', () => {
  it('grades a typical sentence in under 5 ms', () => {
    ...
    const answers = [
      'کتابا رو خیلی دوس دارم',
      'من کتاب ها را دوست دارم',
      'ما کطابا رو خیلی دوست داریم',
      'من نون می‌خوام خیلی زیاد',
      'کتاب',
    ]
    ...
    for (let i = 0; i < runs; i++)
      grade(g, answers[i % answers.length]!, { lang: 'fa', mode: 'typed', lexicon: ['کباب', 'سد'] })
